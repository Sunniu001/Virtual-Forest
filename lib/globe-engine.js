...
export class GlobeEngine {
  constructor(container, options = {}) {
    ...
    // Dynamic Viewport Altitude tracking
    this.targetCameraZ = 33;
  ...
  init() {
    ...
    this.scene.fog = new THREE.FogExp2('#222222', 0.012);

    // 2. Camera setup
    this.camera = new THREE.PerspectiveCamera(45, this.width / this.height, 0.1, 1000);
    this.camera.position.z = 33.0; // Increased distance slightly to shrink globe visual scale per request
  ...
  buildGlobe() {
    ...
    // Original realistic material — white base so texture renders with full natural contrast
    this.sphereMat = new THREE.MeshPhongMaterial({
      color:     0xffffff,
      specular:  0x111111, // near-black: almost no specular glint
      shininess: 12,
    });
    ...
    loader.load(
      ...
      undefined,
      (err) => console.warn('[GlobeEngine] Texture load failed:', err)
    );
  ...
  // ── Holographic texture generator ───────────────────────────────────────
  // Paints a Mercator world map onto a Canvas, returns a THREE.CanvasTexture.
  // Cost: runs once at startup; Three.js GPU-uploads it and never touches CPU again.
  _buildHolographicTexture(texW, texH) {
    ...
    // D3 Mercator maps perfectly to an equirectangular sphere texture
    ...
    // Land fill — we need GeoJSON; fetch it async and update texture
    // For the initial frame, land is rendered as a dark shape via stored data.
    // We trigger an async load and update the texture when ready.
    this._loadAndPaintLand(canvas, ctx, projection, path);

    return new THREE.CanvasTexture(canvas);
  }

  async _loadAndPaintLand(canvas, ctx, projection, path) {
    try {
      ...
      // Repaint full texture with land
      ctx.fillStyle = '#020b18';
      ctx.fillRect(0, 0, texW, texH);
      ...
      // Mark texture as needing GPU re-upload
      if (this.sphereMat && this.sphereMat.map) {
        this.sphereMat.map.needsUpdate = true;
      }
    } catch (e) {
      console.warn('[GlobeEngine] Failed to paint land texture:', e);
    }
  }
  ...
  buildAtmosphere() {
    ...
    this.atmosMat = new THREE.ShaderMaterial({
      ...
      uniforms: {
        color: { value: new THREE.Color(0x334155) } // Original slate-blue atmosphere
      },
      blending: THREE.AdditiveBlending,
      side: THREE.BackSide,
      transparent: true,
  ...
  setUserHouseId(houseId) {
    this.userHouseId = houseId;
    // Dynamically pull back planetary view if allied to provide tactical header space
    this.targetCameraZ = houseId ? 43 : 33; 
  ...
  updateParticleVisuals() {
    ...
    const activeHouseId = this.hoveredHouseId || this.userHouseId;
    
    if (activeHouseId) {
      const h = this.houses.find(house => house.id === activeHouseId);
      if (h) targetAtmosColor.setHex(h.colorHex);
    }
    ...
    Object.keys(this.points).forEach(id => {
      ...
      const isUserHouse = id === this.userHouseId;
      const isHovered = id === this.hoveredHouseId;
      const hasUserHouse = !!this.userHouseId;

      const baseColor = new THREE.Color(house.colorHex);
      const greyColor = new THREE.Color(0x475569); // Muted slate for non-allied particles

      let finalColor = baseColor;
      let finalOpacity = 0.85;
      let intensity = 1.0;
      ...
      if (hasUserHouse) {
        if (isUserHouse) {
          ...
          finalOpacity = 1.0;
          intensity = 1.25; // High emission
        } else {
          finalColor = greyColor;
          finalOpacity = isHovered ? 0.35 : 0.15; // Desaturate non-allied factions
          intensity = isHovered ? 0.55 : 0.35;
        }
      } else {
        if (this.hoveredHouseId) {
          if (isHovered) {
            finalColor = baseColor;
            finalOpacity = 1.0;
            intensity = 1.25;
      ...
      pts.material.opacity = finalOpacity;
      const colorAttr = pts.geometry.attributes.color;
      for (let i = 0; i < colorAttr.array.length; i += 3) {
        colorAttr.array[i] = finalColor.r * intensity;
        colorAttr.array[i+1] = finalColor.g * intensity;
        colorAttr.array[i+2] = finalColor.b * intensity;
      }
    ...
    if (this.atmosphere) {
      ...
      this.atmosphere.scale.setScalar(1 + (activeHouseId ? 0.015 : 0));
    }
  }
    ...
    this.onPointerUp = () => {
      ...
      if (this.dragDistance < 5) {
        ...
        if (intersects.length > 0) {
          ...
          // GATED: Ignore click events from unauthorized sectors when allied
          if (this.userHouseId && houseId !== this.userHouseId) return;
  ...
  animate() {
    ...
    // 3.5 Dynamic Viewport Z lerping (orbital pullback)
    ...
    if (intersects.length > 0) {
      ...
      // GATED: Do not visually recognize hover over foreign factions when allied
      const isAuthorized = !this.userHouseId || hoveredId === this.userHouseId;
      ...
      if (isAuthorized) {
        ...
        // Treat as empty space for foreign sectors
